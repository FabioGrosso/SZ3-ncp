...
class HuffmanEncoder : public concepts::EncoderInterface<T> {
   public:
    ...
    size_t size_est() override{
        ...
        return 1 + 2 * nodeCount * b + nodeCount * sizeof(unsigned char) + nodeCount * sizeof(T) + sizeof(int) +
               sizeof(int) + sizeof(T) + markerSizeEst;
    }
    ...
    size_t encode(const T *bins, size_t num_bin, uchar *&bytes) {
        const MarkerConfig markerConfig = getMarkerConfig(num_bin);
        if (markerConfig.enabled) {
            return encode_marked(bins, num_bin, markerConfig, bytes);
        }
    ...
    std::vector<T> decode(const uchar *&bytes, size_t targetLength) override {
        ...
        MarkerConfig markerConfig = getMarkerConfig(targetLength);
        if (markerConfig.enabled) {
            return decode_marked(bytes, targetLength, encodedLength, markerConfig);
        }
    ...
    std::vector<T> decode_marked_chunks(const uchar *&bytes, size_t targetLength,
                                        const std::vector<size_t> &rowMajorChunks) {
        const size_t encodedLength = *reinterpret_cast<const size_t *>(bytes);
        bytes += sizeof(size_t);
        const MarkerConfig markerConfig = getMarkerConfig(targetLength);
        if (!markerConfig.enabled) {
            bytes -= sizeof(size_t);
            return decode(bytes, targetLength);
        }
   ...
   private:
    ...
    size_t marker_header_size(const MarkerConfig &config) const {
        if (!config.enabled) return 0;
        return config.chunkCount * sizeof(uint64_t);
    }
    ...
    void write_marker_header(uchar *header, const MarkerConfig &config, const std::vector<uint64_t> &bitOffsets) const {
        memcpy(header, bitOffsets.data(), config.chunkCount * sizeof(uint64_t));
    }

    const uint64_t *read_marker_header(const uchar *&bytes, const MarkerConfig &config) const {
        const uint64_t *bitOffsets = reinterpret_cast<const uint64_t *>(bytes);
        bytes += marker_header_size(config);
        return bitOffsets;
    }

    void encode_one_symbol(const T *bins, size_t index, uchar *&p, size_t &outSize,
                           size_t &totalBitSize, int &lackBits) {
        unsigned char bitSize = 0, byteSize, byteSizep;
        int state = bins[index] - offset;
        bitSize = huffmanTree->cout[state];
    ...
    T decode_one_symbol_from_bit(const uchar *bitstream, size_t &bitIndex, node root) const {
        node n = root;
        while (true) {
            const size_t byteIndex = bitIndex >> 3;
            const int r = bitIndex % 8;
            if (((bitstream[byteIndex] >> (7 - r)) & 0x01) == 0)
                n = n->left;
            else
                n = n->right;
            bitIndex++;
    ...
    std::vector<T> decode_marked_chunks_impl(const uchar *&bytes, size_t encodedLength,
                                             const MarkerConfig &config,
                                             const std::vector<size_t> &rowMajorChunks) {
        const uint64_t *bitOffsets = read_marker_header(bytes, config);
        const uchar *bitstream = bytes;
